...
// Manages loading of daily student attendance statistics from phichaia_student database

class Attendance {
    private $db;

    public function __construct() {
        $this->db = Student::connect(); // Reuse PDO connection method from Student Model
    }

    /**
     * Fetch active term code
     */
    public function getTerm() {
        try {
            ...
            error_log("Attendance::getTerm error: " . $e->getMessage());
            return '1';
        }
    }
    ...
    public function getPee() {
        try {
            ...
            error_log("Attendance::getPee error: " . $e->getMessage());
            return date('Y') + 543;
        }
    }

    /**
     * Fetch attendance stats (today, week, month)
     */
    public function getAttendanceStats($date, $term, $year) {
        ...
        try {
            ...
            error_log("Attendance::getAttendanceStats query error: " . $e->getMessage());
        }
    ...
    /**
     * Fetch student enrollment counts
     */
    public function getStudentCounts() {
        ...
        try {
            ...
            error_log("Attendance::getStudentCounts query error: " . $e->getMessage());
        }
    ...
    /**
     * Fetch room-by-room attendance details
     */
    public function getClassAttendance($date, $term, $year) {
        ...
        try {
            $stmt = $this->db->prepare("
                ...
                ORDER BY CAST(s.Stu_major AS UNSIGNED) ASC, CAST(s.Stu_room AS UNSIGNED) ASC
            ");
            $stmt->execute([
                ':date' => $date,
                ':term' => $term,
            ...
            error_log("Attendance::getClassAttendance query error: " . $e->getMessage());
        }
